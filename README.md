...
`cinegraph` is a local Rust workspace for downloading, idempotently caching, and importing film datasets into an embedded SQLite warehouse. The current implementation is IMDb-first and includes both a Tantivy-based full-text search layer and an Oxigraph-based RDF/SPARQL graph projection on top of the SQLite warehouse.

Progress is tracked in [docs/roadmap.md](/win/linux/Code/rust/cinegraph/docs/roadmap.md). Milestones 1, 2, and 3 are implemented; later milestones remain planned.
